//
// Created by Camilo on 28/08/2017.
//
...
int pop[POPSIZE][DIMENSIONS + 1];
double operators_probability[POPSIZE][OPERATORSNUMBER];
int new_pop[POPSIZE][DIMENSIONS + 1];
pcg32_random_t rng_global;
...
void *apply_genetic_operators(void *arg);

void one_point_crossover(int new_individuals[2][DIMENSIONS + 1], int individual, pcg32_random_t *rng);

void mutation(int *new_individual, int individual, pcg32_random_t *rng);

void copy_array(const int original[], int pos);
...
int main() {
    struct Thread_Attributes attributes[THREADS];
    ...
    uint64_t seeds_global[2];
    seeds_global[0] = (uint64_t) random();
    seeds_global[1] = (uint64_t) random();
    pcg32_srandom_r(&rng_global, seeds_global[0], seeds_global[1]);
    srandom(initialSeed);
    initPop();
    initProbabilities();
    ...
    for (i = 0; i < ITERATIONS; i++) {
        ...
        for (j = 0; j < THREADS; j++) {
            attributes[j].threadId = j;
            uint64_t seeds[2];
            seeds[0] = (uint64_t) random();
            seeds[1] = (uint64_t) random();
            pcg32_srandom_r(&attributes[j].rng, seeds[0], seeds[1]);
            pthread_create(&thread[j], NULL, (void *) apply_genetic_operators, &attributes[j]);
        }
...
void initPop() {
    ...
    for (i = 0; i < POPSIZE; i++) {
        for (j = 0; j < DIMENSIONS; j++) {
            pop[i][j] = pcg32_boundedrand_r(&rng_global, 2);
        }
...
void *apply_genetic_operators(void *arg) {
    ...
    struct Thread_Attributes attributes = *(struct Thread_Attributes *) arg;
    initIteration = (POPSIZE / THREADS) * attributes.threadId;
    endIteration = initIteration + ((POPSIZE / THREADS) - 1);
    ...
    for (j = initIteration; j <= endIteration; j++) {
        ...
        for (i = 0; i < OPERATORSNUMBER; i++) {
            ...
            random_number = generate_random(attributes.rng);
